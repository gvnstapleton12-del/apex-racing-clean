...
import { getScore, sortByScore, getScoreColor } from '../lib/engine'
        ...
        <div className='p-8 pt-6 space-y-4'>
          {runners.map((runner, index) => {
            ...
            return (
              <div
                key={index}
                className={`rounded-xl border overflow-hidden ${isFirst ? 'border-amber-500/20 bg-gradient-to-r from-amber-500/5 to-transparent' : 'border-white/5 bg-white/[0.03]'}`}
              >
                <div className='p-6'>
                  <div className='flex items-start justify-between gap-6'>
                    <div className='flex-1 min-w-0'>
                      <div className='flex items-center gap-3 mb-2'>
                        <span className={`text-xs font-bold px-2 py-0.5 rounded ${isFirst ? 'bg-amber-500/10 text-amber-400' : 'bg-white/5 text-zinc-500'}`}>#{index + 1}</span>
